...
def calculate_crop_box(img_width, img_height, y_start_pct, y_end_pct, padding_pct=1.0):
    """
    Calculate the bounding box for PIL cropping based on vertical percentages.
    Includes vertical padding and ensures bounds are clamped to image height.
    """
...
def crop_page_assets(page_data, src_img_path, dest_dir, padding_pct=1.0):
    """
...
def main():
    ...
    resized_dir = "/Users/kenyim/.gemini/antigravity/scratch/math-answer-review/resized"
    dest_dir = "/Users/kenyim/.gemini/antigravity/scratch/math-answer-review/images"
    
    json_files = sorted(glob.glob(os.path.join(cache_dir, "page_*_raw.json")))
    if not json_files:
        print("No cache analysis files found.")
        return
    ...
    for json_path in json_files:
        try:
            ...
            crop_page_assets(page_data, src_img_path, dest_dir)
